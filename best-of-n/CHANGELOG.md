...
Eight models under one protocol, and the trend they make.

### The sweep

Same 200 GSM8K problems, same prompt, same temperature, same token budget,
N=64, weights frozen, every answer re-extracted from the raw text rather than
read from a stored field. Every model improved. The gain runs from **+37.3**
points on SmolLM2-1.7B to **+2.4** on Qwen3.8-27B, and it shrinks steadily as
the base model gets better — which is what should happen, and is published as
the whole table rather than as its best row.

Coverage stays above what the vote returns on all eight, by a median of 9
points. Even the strongest model here, at 95.1% from a single sample, is still
failing to return answers it already found.

### What the sweep caught in itself

The 27B first measured **+7.0**, of which only **+0.2** was selection. It
truncated 11.2% of its trajectories at the 1024-token budget the other models
ran comfortably inside, and a truncated trajectory abstains — so almost the
whole apparent gain was abstention avoidance. That is precisely the artefact
the decomposition exists to expose, found in our own benchmark, on the flagship
row.

Raising the budget twice tells the rest of the story:

| budget | truncated | one sample | Best-of-64 | gain | of which selection |
|---:|---:|---:|---:|---:|---:|
| 1024 | 11.2% | 88.5% | 95.5% | +7.0 | +0.2 |
| 2048 | 6.7% | 93.0% | 97.5% | +4.5 | +0.9 |
| **4096** | **3.7%** | **95.1%** | **97.5%** | **+2.4** | **+1.1** |

The baseline rises and the gain falls as the budget grows. A reader given only
the first row would have been given a number three times too large.

A methodological note, because it caught us twice: the token distribution was
measured from runs whose own cap censored it. "Nothing exceeds 1,929 tokens, so
2048 truncates nothing" was read off data where 2048 *was* the maximum by
construction. You cannot measure the tail of a distribution with the instrument
that cuts it.

### Fixed

- **`max_model_len` was hard-coded to 2048** while `max_tokens` was raised to
  3072. That window covers prompt and generation together, so the larger budget
  bought nothing and three hours of GPU time were spent generating tokens the
  engine could never emit. It is derived from the requested budget now.
- **`max_num_seqs` is configurable.** Qwen3.8-27B is a hybrid Mamba/attention
  model: each concurrent sequence needs its own Mamba cache block, and a 48 GB
  card holding FP8 weights has room for 219 against vLLM's default of 256. The
  model loaded on an 80 GB card and failed outright on a 48 GB one — nothing to
  do with the weights fitting, which they do with room to spare.
- **`run_models.py` re-extracts answers from the raw text** rather than reading
  the stored `answer` field. Reading the stored one makes a replay that cannot
  detect an extraction bug, only inherit it — the exact criticism this project
  levels at other people's replays, and it was in our own new script.
- The multi-model table is generated by `sync_docs.py` from the summary JSON,
  so it cannot drift from the data independently of it.

### Note

The 27B row is the FP8 build Qwen publishes, and the label says so. FP8 weights
are 30.9 GB against 55.6, which is a 48 GB card at $0.58/h rather than an 80 GB
one at $2.34. Substituting a quantised build and presenting it as the base
model would be the sort of quiet swap this library was written to catch.
